...
| 4 | 議論履歴共有とターン制御 | 🚧 着手（2026-05-27 設計確定・ブランチ派生） |
| 5 | システムプロンプト整備 | 未着手 |
| 6 | UI整備 | 未着手 |
...
### 3. 応答ブロックセレクタの脆弱性 — 🔥 現実化（2026-05-27）→ ✅ 修正（commit 6c7d1eb）
- **2026-05-27、ついに現実化**: claude.ai が応答 DOM を変更し、抽出が
  `fallback:retry-ancestor-depth-0` で **1 字**しか取れないリグレッション発生
  （画面では正常応答）。Phase 4 Step1 検証中に発見。
- **切り分け**: claude.js が main(47ac601) とバイト無変更／Phase 4 はサイド
  パネル側のみ（claude.ai に注入されない）／claude.js は storage を読まない
  → **Phase 4 無罪**。main でも再現し、claude.ai 側 DOM 変更と確定。
- **原因（DOM ロガーで採取・特定）**: claude.ai は `data-testid="action-bar-copy"`
  / `data-testid="action-bar-retry"` を導入。一方 `assistant-message` /
  `data-message-author-role` は依然不在。旧戦略3（Retry 祖先を上にたどり
  「最初に 1 字以上ある祖先」を採用）が、起点ボタンの直親＝アクションバー
  （アイコンのみ・1 字）で誤打ち切りしていた。本文はその兄弟側にあり、祖先を
  さらに上らないと届かない。**新しい常態 depth は 5→7（DOM が +2 段深化）**。
- **修正（案A）**: 起点を `data-testid="action-bar-retry"/"action-bar-copy"`
  （言語非依存）に。選択を「最初に 1 字」→「user-message を含まない最高位の
  祖先（＝応答ターン全体）」に。selector は `fallback:action-bar-ancestor-depth-N`。
  戦略1/2/4・dedup・完了検知は無変更。chatgpt.js/gemini.js もバイト無変更。
- **副次バグも同時修正**: 旧戦略3 が「1 字」を成功扱いするため抽出が「全滅」に
  ならず、失敗時のスナップショット自動保存が発火しない → 「📂 最新スナップ
  ショット」が無関係な過去（Gemini）の保存物を返す事故。`MIN_ASSISTANT_CHARS`
  (=2) 未満を失敗扱いに落とすことで、真の失敗時にスナップショットが再保存される。
- W1〜W4 警告も新アンカーに整合（W2=新 depth 可視化、W4=aria-label +
  action-bar testid の両者でアンカー全滅判定）。A4 自動 DOM ロガーは不変。
- **学び**: 構造ベース fallback の脆さが顕在化したが、DOM ロガー + スナップ
  ショット + W 警告という観測インフラ（Phase 2 整備）のおかげで 1 セッション内に
  切り分け・採取・修正まで到達できた。「自動修復より観測性」方針の有効性を実証。
...
### 既知の課題#7: 非アクティブタブのスロットルによる送信パイプライン遅延（2026-05-25 観察 → 2026-05-27 原因確定・悪化版を記録）
...
**2026-05-27 悪化版を観察し原因確定**: claude.js 抽出修正の 10 連続検証中、
iteration 9 で**送信パイプライン内に約 6 分の沈黙**（beforeinput 失敗まで +39 秒 /
clipboard-paste 成功まで +2 分 / 送信ボタンヒット +1 分 / dispatch 完了 +2 分）→
応答取れず、iteration 10 は busy 検知でスキップ。**Kazuya 確認: PC は起きていたが
Chrome タブを離れて他作業していた** → **原因は Chrome の非アクティブ／裏タブの
スロットル**（タイマー・処理の間引き）と確定。100 秒級 → 6 分に悪化したのは
タブが完全に裏に回ったため。**拡張のバグではない**。抽出修正の合否とは無関係
（抽出に到達した 8 回 + 単発はすべて depth=7 で正常）。
- **運用条件（重要）**: 自動進行（連続テスト / Phase 4 の自動1周・2周）中は
  **対象タブをアクティブに保つ**こと。裏に回すとスロットルで送信が遅延・停止しうる。
- **Phase 4 への含意**: 自動進行のオーケストレーションはサイドパネル駆動だが、
  各社タブが裏だと注入/送信が間引かれる。Phase 4/8 で「進行中はタブを前面に保つ」
  ガイドや、必要なら `chrome.tabs.update({active:true})` での明示アクティブ化を検討。

**2026-05-31 Phase 4 Step2b で配信時アクティブ化を実装し対処**: `chrome.tabs.update`
で配信直前に対象タブを一時アクティブ化（同 window 内、`windows.update` は呼ばずフォーカス
奪取最小）+ 配信後に元タブ復帰。Step2a の 78〜171秒/社 → 待機指示2回目以降 **16〜20秒/社**
に短縮（実測）。
- **残る運用条件**: Chrome ウィンドウがフォーカス状態のときだけ visibilityState="visible"
  になり throttle 解除される仕様。**配信中に Kazuya が別アプリ（Slack 等）へ alt-tab すると
  Chrome ウィンドウがフォーカス外＝再びスロットル復活**。完全回避には `windows.update({focused:true})`
  が必要だがフォーカス奪取の害が大きく不採用。「配信中は Chrome を見ている前提」が運用条件。

### 既知の課題#8: Claude 応答に稀に英単語混入（2026-05-31 観察、対応不要）
- 現象: Phase 4 Step2b 検証中、Claude が「古代に大陸文化を取り入れ**while**天皇を中心と
  した」のように本文中に英単語を混入させた例を観察。
- 確認: claude.ai の画面でも同じ出力（拡張は正しく抽出している）。**Claude 本体の出力で
  あって拡張側のバグではない**。
- 対応: 不要（Claude 側の言語選択ゆらぎ。観察記録のみ）。
...
1. 3社揃った Roundtable プロンプト Spike 再テスト（Step6 相当。仕様書 §5
   ...
   単体は実証済み、3社での再検証は未実施）→ **Phase 4 Step2 のスパイクに統合**
2. Phase 3b 完了として main マージ判断 → Phase 4（議論履歴共有とターン制御）へ
   → **2026-05-27 完了。47ac601 で main マージ済み。**

---

## Phase 4 着手（議論履歴共有とターン制御）

**着手日**: 2026-05-27 / **作業ブランチ**: feature/phase4-turn-control（main 47ac601 から派生）
**ロードマップ目標**: 3社+Kazuya のラウンドテーブル議論が**技術的に成立**する（見た目の作り込みは Phase 6）

### Phase 3 が残した非対称（Phase 4 設計の出発点）

- **content_scripts 3本（claude/chatgpt/gemini、各 1450〜1600 行）は完成・成熟**。
  各社の送信・完了検知・抽出・Thinking 検知は production ready。
  → **Phase 4 では原則これらを触らない**（バイト差分ゼロでリグレッション安全を証明）。
- **サイドパネルは「単一ターゲットのデバッグ用コンソール」**。対象 AI 1つ + 送信先タブ
  1つで 1 往復するだけ。議論スレッド・複数社配信・ターン制御は未実装。
- ターン制御の足場になる**逐次オーケストレーションは E モード（連続テスト）で実証済み**
  （side_panel.js、10 連続を 200 秒級で完走）。これを流用する。

→ **Phase 4 = 完成済み 3 本の content_script を、複数社オーケストレーション + 議論履歴で
束ねる「層」を新規に作る**。content_script の作り直しではない。

### スコープ確定（2026-05-27 キックオフ、Kazuya 判断）

| 分岐点 | 決定 | 理由 |
|---|---|---|
| **UI 範囲** | **機能優先・最小UI** | ロードマップの Phase 4/6 分担通り。議論スレッド表示 + 3社タブ選択 + 指名/1周/2周/停止を最小実装。Slack 風 UI（4色アバター・ターンカウンタ・Markdown 整形）は Phase 6。ターン制御のバグに集中でき検証が速い |
| **詰まり対応** | **最小（自動スキップ + 停止のみ）** | 自動進行が 1 社のハングで止まらない最低限。仕様書§10 の「再試行/スキップ/中断」3択UIは Phase 6 |

### アーキ判断

1. **ターン制御オーケストレーションはサイドパネル駆動**（E モードの実証パターン流用）。
   MV3 Service Worker は長時間タスクで落ちやすいため、background にステートマシンを
   置く仕様書§3 の記述からは意図的に逸脱。**履歴は storage に永続化するのでパネルを
   閉じても残る**。自動進行中だけはパネルを開いたまま運用。
2. **content_scripts は無変更**（バイト差分ゼロで保証）。
3. **background.js は履歴 storage ヘルパのみ追加**（送信経路 send_to_ai / ping_ai は不変）。

### データモデル（仕様書§7 準拠）

- **Session**: id / title / participants / turns[] / defaultOrder / createdAt / updatedAt / origin
- **Turn**: id / sessionId / speaker('kazuya'|'claude'|'chatgpt'|'gemini') / content / timestamp /
  metadata{ model?, durationMs?, error? }
- **Storage**: `rt_session_<id>`（セッション本体）+ `rt_active_session`（アクティブ ID ポインタ）
- **タブ束縛 / §5 初回投入済みフラグ**は永続化せず**パネル runtime 状態**で持つ
  （tabId はブラウザ再起動で変わるため。セッション継続時は 3 タブを選び直して再投入）

### §5 プロトコル投入タイミング

1. **初回投入プロンプト（§5）**: セッション開始/初回発言時に各タブへ 1 回（panel runtime で投入済み管理）
2. **Kazuya 発言配信（待機指示・会議メタファ型）**: 発言ごとに 3 タブへ。各 AI は「了解」とだけ返す
3. **応答指示（全履歴付き）**: 指名/進行で対象社のタブへ

### ターン制御ステートマシン（panel 内）

- 状態: IDLE（指名待ち）/ DELIVERING（発言を 3 タブ配信中）/ RESPONDING（対象社応答待ち）/
  AUTO（自動進行の逐次ループ）/ STOPPING
- 進行モード: 指名（1社）/ 自動1周（defaultOrder 順 = Gemini→ChatGPT→Claude）/ 自動2周
- 停止: AUTO ループのステップ境界でフラグチェック（E モードと同型）
- スキップ: 無音タイムアウト/エラーで自動スキップ（自動進行をハングさせない）

### Phase 4 ステップ計画（Phase 3 と同じ「調査↔実装の分離 + 手動検証ゲート」）

| Step | 内容 | 種別 | 検証ゲート |
|---|---|---|---|
| **Step1** | データモデル + 履歴永続化 + 3社タブ保持 + 最小議論スレッド表示 | 実装 | 発言が履歴に積まれ表示順正しい / パネル再開後も残る / 3 タブに ping 到達 / content_scripts バイト差分ゼロ |
| **Step2** | 配信 + §5 プロトコル投入。**3社が「了解」だけ返して暴走しないか実機スパイク**（Phase 3 持ち越しの 3社§5検証の核心。仕様書§12「勝手に応答開始」リスク検証） | 実装+調査 | 初回投入 + 待機指示が 3 タブに届く（目視）/ 3社が「了解」のみ返す（暴走なし） |
| **Step3** | ターン制御（指名/自動1周/自動2周/停止/スキップ） | 実装 | ロードマップ Phase 4 完了条件 全項目 |

### Phase 4 Step1 + Step1.5 完了（2026-05-31 Kazuya 実機検証 合格）

途中で **claude.ai の DOM 変更による既知の課題#3 が現実化**（depth=0,1字 リグレッション）。
Phase 4 無罪を git 切り分けで確定後、claude.js を action-bar testid 起点へ修正
（main 1e757e0 にマージ済み、新常態 depth=7）。Phase 4 ブランチを新 main に再ベース
ライン後、Step1 を再検証して合格。続いて Step1.5 を Kazuya 要望で追加した。

**Step1 実装**: 議論履歴データモデル（仕様書§7 Session/Turn）を新規 lib/session_store.js
に。Phase 4 パネル UI（セッション管理 + 3社タブ束縛 + 議論スレッド + Kazuya 発言記録）
を新規 lib/roundtable_panel.js + side_panel.html 加筆で実装。**side_panel.js と
content_scripts は無変更**（Step1 隔離設計）。

**Step1.5 実装**: タブ自動選択ルール改善。background.js の listAiTabs 並び順を
「**同 Chrome ウィンドウ最優先 → lastAccessed 新しい順 → tabId 大きい順**」に差し替え
（旧並びは「アクティブ1枚優先 + tabId 昇順」で、フォーカス外の社が別ウィンドウの
最古タブを掴む事故があった）。共有レイヤを直したのでラウンドテーブル 3 ドロップダウン
+ 既存デバッグコンソール**両方が自動改善**。side_panel.js / content_scripts は無変更。
...
| ② 3タブ ping | ✅ 3社とも到達 |
| ③ 発言記録（履歴に積まれる）| ✅ |
| ④ 永続化 + 再オープン時タブ選択 | ✅ 前回の「別タブに飛ぶ」事故解消 |
| ⑤ Claude リグレッション | ✅ depth=7, 27字（action-bar 修正動作継続） |

**TODO（軽微・Phase 6 ついでで可）**: サイドパネル起動ログの「(先頭にフォールバック)」
文言が、新しい並び順の先頭を選んだ正常ケースでも出るので紛らわしい。「同 window 最近
タブを選択」等に文言調整。
→ **Step1.6 で同種を整理（"（同 window の最近タブを自動選択）" など新文言を導入）**。
残る古い「(先頭にフォールバック)」は同 window 候補あり時の例外パスのみで発火するため
実害ほぼ無し。完全整理は Phase 6 で。

### Phase 4 Step1.6 + Step2a + Step2b 完了（2026-05-31 Kazuya 実機検証 合格・1コミット）

途中で claude.js 抽出の二次バグ（claude.ai の新 button ラベル末尾混入＋それを直す過程で
detached node の innerText が dedup を壊す副作用）が発見・main で hotfix 済み
（commit `eeabba8` → main `2eca14b` → phase4 へ merge `ef8a8cc`）。その上で Phase 4 内
panel UX 層を一気に完成させた。

**Step1.6 実装（タブ選択厳格化）**: Step1.5 の sort（同 window 最優先）の上で、同 window
候補ゼロのケースを発見（Gemini が別 window のみ＝古い別 window タブを fallback 選択して
content_script 未注入＝Step3 自動進行で配信壊滅の致命性）。background.js が
`sameWindowCount` を返し、panel 側で「同 window 候補ゼロなら自動選択しない・プレースホルダ
表示・送信ボタン無効」policy に変更。手動選択時は `tabSelect.change` リスナーでボタン
復活。共有レイヤを直したのでデバッグコンソール+ラウンドテーブル両方が自動改善。

**Step2a 実装（§5 初回投入スパイク）**: 仕様書§5 文面を `lib/prompts.js` に定数化
（initial / standby / response の3種）。「🧪 §5 初回投入を 3 社へ (Step2a)」テストボタン
で 3 社に逐次配信。Kazuya 実機: Gemini 69字「了解しました。集団思考や過度な迎合を排し、
独立した一人の参加者として本音で議論に臨みます。Kazuya議長、最初のお題をお願いします。」/
ChatGPT 32字「了解。指示が来たら、ChatGPT参加者として簡潔に応答します。」/ Claude 46字
「了解しました。Claudeとして参加します。指示が来るまで待機します。」全社が短い理解応答に
留まり暴走ゼロ、各社が§5 役割を正しく認識。**Phase 3 持ち越しの「3社§5プロトコル検証
（必要条件）」実証クローズ**。

**Step2b 実装（Kazuya 発言時の本フロー）**: 「発言を記録」押下で ① Kazuya 発言を履歴に積む
② 未投入の社に §5 初回投入 ③ 3社へ standbyPrompt（会議メタファ型待機指示）を**逐次配信**
④ 各社の「了解」応答を Turn として記録（`metadata.kind="standby_ack"`）。失敗社は自動
スキップ。**課題#7 対処**: 配信直前に `chrome.tabs.update({active:true})` で対象タブを
一時アクティブ化してスロットル回避（同 window 内、`windows.update` は呼ばずフォーカス
奪取最小）+ 配信後に元タブ復帰。Step2a の 78〜171秒/社 → 待機指示2回目以降 **16〜20秒/社**
に短縮（案A 効果実証）。

**連続スパイクの収穫（Step2b）**: Kazuya 発言 4 ターン通して 3 社全部が「了解」相当を
維持。Gemini「了解。」3字 / ChatGPT「了解」2字 / Claude「了解」13字 規模。ふざけた入力
（「こんにちわんこそば」等）でも待機を崩さず暴走ゼロ。**会議メタファ型待機指示が連続・
入力内容非依存で効くと実証**。仕様書§12「勝手に応答開始」リスクは Phase 4 通して一度も
顕在化せず。

**検証結果（Kazuya 実機、Step1.6+Step2 全項目 ✅）**:

| 項目 | 結果 |
|---|---|
| A-1 同 window 有り→自動選択+ボタン有効 | ✅ |
| A-2 同 window 無し→プレースホルダ+ボタン無効（別 window タブ件数表示）| ✅ 再発バグ構造的解消 |
| A-3 手動選択でボタン復活 | ✅ change リスナー効果 |
| B-1 3 社配信→3 つの standby_ack Turn | ✅ |
| B-2 1 社欠け→自動スキップ（成功2/失敗0/スキップ1）| ✅ |
| C Claude depth=7・[C1] dedup 発火・重複/末尾ゴミなし | ✅ trim fix 動作継続 |
| C ChatGPT / Gemini リグレッションなし | ✅ |

**コミット**: 1 つの phase4 コミットで Step1.6 + Step2a + Step2b をまとめる
（panel UX 層の連動した完成として）。content_scripts 3本は main(`2eca14b`) と
バイト完全一致を git diff で証明。

### Phase 4 残り = Step3（ターン制御）

Step2 までで「履歴 + 配信 + 待機」基盤が揃った。Step3 で「指名 / 自動1周 / 自動2周 /
