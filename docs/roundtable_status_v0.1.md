...
1. **Phase 4 Step1 実装**（データモデル + 履歴永続化 + 3社タブ保持 + 最小議論スレッド表示）
   - content_scripts は無変更（バイト差分ゼロを git diff で証明）
   - 検証ゲート: 発言が履歴に積まれ表示順正しい / パネル再開後も残る / 3 タブに ping 到達
2. Step1 の Kazuya 手動検証 ✅ 後に Step2（配信 + §5 スパイク）→ Step3（ターン制御）
3. Phase 4 完了条件（ロードマップ）達成で main マージ判断 → Phase 5（システムプロンプト整備）へ
