...
1. **新セッションで Phase 2 開始**（コンテキスト整理のため）
2. Phase 2 のスコープを仕様書 v0.4 / ロードマップ v0.1 で再確認
3. Phase 1 で見えた既知課題の優先度判断:
   - 「長文応答の重複出力」を Phase 2 のスコープに含めるか
   - 「TipTap 注入順序変更」を Phase 2 で実施するか
   - Phase 2 本来のスコープ（DOM操作の堅牢化）との兼ね合い検討
4. Phase 2 完了条件をクリアしてから Phase 3（3社対応）へ
